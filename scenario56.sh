...
TEST_SCENARIO=$4
DURATION=$5
BANDWIDTH_DOWNLOAD_LIMIT=$6
BANDWIDTH_UPLOAD_LIMIT=$7
NETWORK_CONDITION=$8
...
if [[ $TEST_SCENARIO -eq 3 ]] && [[ -n $NETWORK_CONDITION ]]; then
	RESULT_DIR="n$TOTAL_CLIENTS/scenario-5/$NETWORK_CONDITION/"
elif [[ $TEST_SCENARIO -eq 4 ]] && [[ -n $NETWORK_CONDITION ]]; then
	RESULT_DIR="n$TOTAL_CLIENTS/scenario-6/$NETWORK_CONDITION/"
else
	RESULT_DIR="n$TOTAL_CLIENTS/scenario-$TEST_SCENARIO/$NETWORK_CONDITION/"
fi
...
apply_bandwidth_limit() {
	...
	sudo tc qdisc add dev ens4 root handle 1: htb default 30
	sudo tc class add dev ens4 parent 1:1 classid 1:20 htb rate $1
	sudo tc filter add dev ens4 protocol ip parent 1: prio 1 u32 match ip src 0.0.0.0/0 flowid 1:20
	sudo tc qdisc add dev ens4 handle ffff: ingress
	sudo tc filter add dev ens4 protocol ip parent ffff: prio 50 u32 match ip src 0.0.0.0/0 police rate $2 burst 15k drop flowid :1
}
...
ssh -f -o StrictHostKeyChecking=no $USERNAME@$SERVER_EXTERNAL_IP "$(declare -f apply_bandwidth_limit); apply_bandwidth_limit $BANDWIDTH_UPLOAD_LIMIT $BANDWIDTH_DOWNLOAD_LIMIT"
...
for client in "${clients[@]}"; do
    IFS=':' read -r INSTANCE_NAME EXTERNAL_IP ZONE <<< "$client"
    ssh -f -o StrictHostKeyChecking=no $USERNAME@$EXTERNAL_IP "$(declare -f apply_bandwidth_limit); apply_bandwidth_limit $BANDWIDTH_UPLOAD_LIMIT $BANDWIDTH_DOWNLOAD_LIMIT"
done
...
for VARIANT in "${variants[@]}"; do
	...
	for client in "${clients[@]}"; do
        ...
        ssh -f -o StrictHostKeyChecking=no $USERNAME@$EXTERNAL_IP "$(declare -f remove_bandwidth_limit); remove_bandwidth_limit"
	done
done

ssh -f -o StrictHostKeyChecking=no $USERNAME@$SERVER_EXTERNAL_IP "$(declare -f remove_bandwidth_limit); remove_bandwidth_limit"
